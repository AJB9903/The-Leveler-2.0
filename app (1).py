...
def extract_text_from_pdf(pdf_bytes: bytes) -> str:
    """Extract plain text from PDF using pypdf — free, instant, no API cost."""
    try:
...
def call_claude_text(client, prompt: str, max_tokens: int = 2000) -> str:
    """Simple text-only Claude call — cheapest possible."""
...
def extract_scope_from_drawings(pdf_bytes: bytes, trades: list) -> dict:
    """Extract text from PDF once, then query Claude per trade (text-only = cheap)."""
    client = get_client()
    if not client:
        return {}
    ...
    # Step 1: Extract text locally — FREE, no API call
    pdf_text = extract_text_from_pdf(pdf_bytes)
    if not pdf_text or len(pdf_text) < 100:
        st.warning("⚠ Could not extract text from this PDF. It may be a scanned/image-only PDF.")
        progress.empty()
        return result
    ...
    progress.progress(10, text="Text extracted — querying Claude per trade…")

    # Step 2: One Claude text call per trade (cheap — text tokens only)
    for i, trade in enumerate(trades):
        pct = 10 + int((i / len(trades)) * 80)
        progress.progress(pct, text=f"Extracting scope: {trade} ({i+1}/{len(trades)})…")

        prompt = f"""You are a senior construction estimator reviewing construction drawing documents.
...
List every scope item required for {trade} visible in these drawings.
Be specific: materials, product types, finishes, dimensions, quantities where mentioned.
...
Keep each item under 15 words. Return 5-25 items. If {trade} has no visible scope, return {{"scope_items": []}}"""

        try:
            raw = call_claude_text(client, prompt, max_tokens=1500)
            raw = repair_json(raw)
            parsed = json.loads(raw)
            result["trades"][trade] = parsed.get("scope_items", [])
        except json.JSONDecodeError as e:
            st.warning(f"⚠ Could not parse scope for {trade} — add manually. ({e})")
            result["trades"][trade] = []
        except Exception as e:
            st.warning(f"⚠ API error on {trade}: {e}")
            result["trades"][trade] = []

    # Step 3: Suggested trades — one more text call
    progress.progress(93, text="Checking for additional required trades…")
    trades_str = ", ".join(trades)
    suggest_prompt = f"""You are a construction estimator reviewing drawing documents.

DRAWING TEXT (excerpt):
{pdf_text[:20000]}

---
Trades already defined: {trades_str}

List any OTHER trades clearly required by these drawings but NOT in the list above.
Maximum 5 suggestions. If none needed, return an empty array.
...
{{
  "suggested_trades": [
    {{
      "name": "Trade Name",
      "reason": "One sentence why it is needed",
    ...
    try:
        raw2 = call_claude_text(client, suggest_prompt, max_tokens=800)
        raw2 = repair_json(raw2)
...
def parse_bid_pdf(pdf_bytes: bytes, trade: str, master_scope: list, sub_name: str) -> dict:
    """Parse a subcontractor bid PDF — text extraction then two Claude text calls."""
    client = get_client()
    if not client:
        return {}

    # Extract text locally — FREE
    bid_text = extract_text_from_pdf(pdf_bytes)
    if not bid_text or len(bid_text) < 50:
        st.warning("⚠ Could not extract text from bid PDF. It may be a scanned document.")
        bid_text = "[No text could be extracted from this PDF]"

    if len(bid_text) > 60000:
        bid_text = bid_text[:60000] + "\n[... truncated ...]"
    ...
    # Pass 1 — extract price, inclusions, exclusions, notes
    prompt1 = f"""You are a construction bid analyst reviewing a subcontractor bid.
...
Extract the following. Respond ONLY with valid compact JSON (no markdown, no explanation):
{{
  "subcontractor_name": "{sub_name}",
  "total_price": 0,
  ...
  "key_notes": ["note 1"],
  "bid_confidence": "HIGH",
  "bid_confidence_reason": "brief reason"
}}

Rules:
- total_price = plain number only (no $ signs or commas)
- Keep each inclusion/exclusion under 12 words
- Max 20 inclusions, 10 exclusions, 5 notes
- bid_confidence: HIGH = clear complete scope, MEDIUM = some ambiguity, LOW = vague or missing scope"""

    try:
        raw1 = call_claude_text(client, prompt1, max_tokens=2000)
        raw1 = repair_json(raw1)
        p1 = json.loads(raw1)
        for k in p1:
            if k in result:
                result[k] = p1[k]
    except json.JSONDecodeError as e:
        st.warning(f"⚠ Bid parse issue — partial data saved. ({e})")
    except Exception as e:
        st.error(f"API error reading bid: {e}")
        return result

    # Pass 2 — gap detection (text-only, no PDF needed)
    if master_scope:
        inclusions_text = "\n".join(f"- {item}" for item in result.get("inclusions", [])) or "None stated"

        prompt2 = f"""You are a construction estimator doing scope gap analysis.

MASTER SCOPE (all required items for {trade}):
{scope_list}

SUBCONTRACTOR'S STATED INCLUSIONS:
{inclusions_text}

Identify every Master Scope item that is NOT clearly covered by the subcontractor's inclusions.
...
Risk: HIGH = definite change order risk, MEDIUM = worth clarifying, LOW = minor"""

        try:
            raw2 = call_claude_text(client, prompt2, max_tokens=2000)
            raw2 = repair_json(raw2)
            p2 = json.loads(raw2)
...
with main_tabs[2]:
    ...
    <div class='card'>
        ...
        <p style='color:#94A3B8; font-size:0.84rem;'>
            ...
            Claude Sonnet is used for all analysis — high accuracy at a fraction of the cost of Opus.
        </p>
    </div>
    """, unsafe_allow_html=True)
